...
export class SeedConfig {
  ...
  */
   ...
   * @type {boolean}
   */
  ENABLE_SCSS: boolean = ['true', '1'].indexOf(`${process.env.ENABLE_SCSS}`.toLowerCase()) !== -1 || <boolean>argv['scss'] || false;
  ...
  NPM_DEPENDENCIES: InjectableDependency[] = [
    ...
    //{ src: '.tmp/rxjs.min.js', inject: 'libs', buildType: BUILD_TYPES.DEVELOPMENT },
  ];
  ...
  SYSTEM_BUILDER_CONFIG: any = {
    ...
    packageConfigPaths: [
      ...
      join('node_modules', '@mapbox', '*', 'package.json'),
      join('node_modules', '@ngrx', '*', 'package.json'),
      join('node_modules', '@turf', '*', 'package.json')
    ],
    paths: {
