...
export class SeedConfig {
  ...
  get DEPENDENCIES(): InjectableDependency[] {
    return normalizeDependencies(this.NPM_DEPENDENCIES.filter(filterDependency.bind(null, this.BUILD_TYPE))).concat(
   ...
   * @type {Array}
   */
  BROWSER_LIST = ["ie >= 10", "ie_mob >= 10", "ff >= 30", "chrome >= 34", "safari >= 7", "opera >= 23", "ios >= 7", "android >= 4.4", "bb >= 10"];
  ...
  getKarmaReporters(): any {
    return {
      ...
      coverageReporter: {
        ...
        reporters: [{ type: "json", subdir: ".", file: "coverage-final.json" }, { type: "html", subdir: "." }]
      },
