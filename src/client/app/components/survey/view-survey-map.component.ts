...
@Component({
  ...
  templateUrl: 'view-survey-map.component.html',
  styleUrls: ['../maps.css']
...
export class ViewSurveyMapComponent implements OnInit, OnChanges {
  ...
  setStations(platform: Platform) {
    ...
    let ls = Turf.featureCollection(this.stations.map(station => MapService.getFeature(station, { code: station.properties.code, hasCount: this.stationsCount.includes(station) })))
    this.layerStations$ = of(ls);
    this.bounds = MapService.zoomToStations(ls);
  }

  setStationsCount(stations: Station[]) {
    let fsc = Turf.featureCollection(stations.map(station => Turf.point(station.geometry.coordinates, { code: station.properties.code })));
    this.layerStationsCount$ = of(fsc);
    this.bounds = MapService.zoomToStations(fsc);
  } 
